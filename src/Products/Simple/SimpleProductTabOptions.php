...
class SimpleProductTabOptions {
	...
	public function __construct( $productManager ) {
		...
		add_filter( 'custom_price_display/products/simple/config', array( $this, 'loadSettings' ), 20, 2 );
	}

	public function render( ProductTab $tab, \WP_Post $post ) {
		$customPriceData = get_post_meta( $post->ID, '_cpdfw_simple_custom_price_data', true );
		...
		<div class="hidden <?php echo esc_attr( $classes ); ?> options_group">
			...
			<?php
				...
				$tab->renderInputOption( array(
						...
						'id'          => 'cpdfw_simple_product_price_suffix',
						'value'       => $priceSuffix,
				...
				$tab->renderWPEditor( array(
						...
						'id'    => 'cpdfw_simple_product_custom_price_template',
						'value' => $priceTemplate,
				) );
			?>
		</div>
	...
	public function save( $tab, $productId, $data ) {
		$priceFormat   = isset( $data['cpdfw_simple_product_custom_price_format'] ) ? sanitize_text_field( wp_unslash( $data['cpdfw_simple_product_custom_price_format'] ) ) : '';
		$pricePrefix   = isset( $data['cpdfw_simple_product_price_prefix'] ) ? sanitize_text_field( wp_unslash( $data['cpdfw_simple_product_price_prefix'] ) ) : '';
		$priceSuffix   = isset( $data['cpdfw_simple_product_price_suffix'] ) ? sanitize_text_field( wp_unslash( $data['cpdfw_simple_product_price_suffix'] ) ) : '';
		$priceTemplate = isset( $data['cpdfw_simple_product_custom_price_template'] ) ? wp_kses_post( wp_unslash( $data['cpdfw_simple_product_custom_price_template'] ) ) : '';
		...
		if ( $priceFormat ) {
			...
			update_post_meta( $productId, '_cpdfw_simple_product_custom_price_data', $priceFormatData );
		} else {
			delete_post_meta( $productId, '_cpdfw_simple_product_custom_price_data' );
		}
	}

	public function loadSettings( $config, $productId ): array {

		$productData = get_post_meta( $productId, '_cpdfw_simple_product_custom_price_data', true );
